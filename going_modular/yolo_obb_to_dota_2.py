...
---------------------
By default the converted annotations land **beside** the split's existing
``images/`` and ``labels/`` folders::

    <root>/
        data.yaml
        conversion_manifest.json      <- written here
        train/  images/  labels/  ann/    <- ann/ created here
        valid/  images/  labels/  ann/
        test/   images/  labels/  ann/

That is what ``ConvertConfig.output_root = None`` (the default) means. It
requires ``root`` to be writable, so point it at a copy under
``/kaggle/working`` rather than at the read-only ``/kaggle/input`` mount.

Set ``output_root`` only when you want the annotations somewhere else
entirely; they then go to ``<output_root>/<split>/<ann_dirname>/`` and the
manifest to ``<output_root>/<manifest_name>``.

Layout assumptions
------------------
The default layout is the Roboflow export shape (``train``/``valid``/``test``,
each with ``images/`` and ``labels/``). Every part of that is overridable: the
split folder names, the images folder name, the labels folder name --
globally or per split, by name or by absolute path. See :class:`DatasetLayout`.

Nothing is ever deleted or modified in the source tree.
...
::
    ...
    cfg = ConvertConfig()            # output_root stays None -> in-place ann/
    manifest = convert_dataset(layout, cfg)

    # -> /kaggle/working/PCB-Defect-6/test/ann/*.txt

Command line::

    python yolo_obb_to_dota.py --root /kaggle/working/PCB-Defect-6

Omit ``--output`` for the in-place layout above; pass it only to redirect
the annotations elsewhere.
"""
...
READ_ONLY_PREFIXES: Tuple[str, ...] = ("/kaggle/input",)
...
class DatasetLayout:
    ...
    Default Roboflow shape::
        ...
        DatasetLayout(root="/kaggle/working/PCB-Defect-6")

    Only the splits that exist (a tiled set may have no ``valid``)::
...
class ConvertConfig:
    ...
    """Where converted annotations go.

    ``None`` (the default) writes ``<root>/<split>/<ann_dirname>/`` -- that is,
    an ``ann/`` folder sitting beside each split's existing ``images/`` and
    ``labels/``. This needs ``root`` to be writable, so it fails on read-only
    mounts such as ``/kaggle/input``; copy the dataset into ``/kaggle/working``
    first.

    Set it to a path only when the annotations should live somewhere separate
    from the images; output then goes to ``<output_root>/<split>/<ann_dirname>/``.
    """
    ...
    def ann_dir_for(self, layout: "DatasetLayout", split: str) -> Path:
        """Where this split's DOTA files are written.

        In-place (``output_root is None``) puts ``ann/`` beside the split's
        ``images/`` and ``labels/``; otherwise it mirrors the split structure
        under ``output_root``.
        """
        if self.output_root is None:
            return layout.split_dir(split) / self.ann_dirname
        return self.output_root / split / self.ann_dirname

    def manifest_dir_for(self, layout: "DatasetLayout") -> Path:
        """Where the JSON manifest is written -- the dataset root when in-place."""
...
def convert_split(
    ...
    With the default ``output_root=None`` the DOTA files land in
    ``<root>/<split>/ann/``, beside that split's ``images/`` and ``labels/``.
    """
    labels_dir = layout.labels_dir(split)
    images_dir = layout.images_dir(split)
    output_dir = cfg.ann_dir_for(layout, split)
    ...
    if not cfg.dry_run:
        try:
            ...
            raise OSError(
                f"cannot create {output_dir} ({exc}). The dataset root is not writable -- "
                f"copy it under /kaggle/working, or set ConvertConfig(output_root=...) "
                f"to redirect the annotations."
            ) from exc
...
def convert_dataset(
    ...
    cfg:
        Conversion options (see :class:`ConvertConfig`). The default writes
        each split's DOTA files to ``<root>/<split>/ann/``.
    splits:
    ...
    -------
    dict
        The manifest, also written to ``<root>/<manifest_name>`` (or under
        ``output_root`` when that is set) unless ``dry_run``.
    """
    cfg = cfg or ConvertConfig()

    if cfg.output_root is None and str(layout.root).startswith(READ_ONLY_PREFIXES):
        logger.warning(
            "root %s looks read-only; in-place output will fail. Copy the dataset "
            "into /kaggle/working or set ConvertConfig(output_root=...).",
            layout.root,
        )
    ...
    if not cfg.dry_run:
        manifest_root = cfg.manifest_dir_for(layout)
        manifest_root.mkdir(parents=True, exist_ok=True)
        manifest_path = manifest_root / cfg.manifest_name
...
def verify_conversion(
    ...
    labels_dir = layout.labels_dir(split)
    images_dir = layout.images_dir(split)
    ann_dir = cfg.ann_dir_for(layout, split)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "By default each split's annotations are written to <root>/<split>/ann/, "
            "beside that split's images/ and labels/. Pass --output only to redirect "
            "them elsewhere (necessary when the root is read-only, e.g. /kaggle/input)."
        ),
    )
    parser.add_argument("--root", required=True, help="dataset root containing the split folders")
    parser.add_argument("--output", default=None,
                        help="redirect annotations to <output>/<split>/<ann-dirname>/ "
                             "instead of writing them beside the images (default)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for split, summary in manifest["splits"].items():
        print(f"  {split} -> {summary['output_dir']}")
